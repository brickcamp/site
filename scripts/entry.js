...
export function createEntry(slug) {
  ...
  // The two fields are set by name, so a renamed one throws on its own. The
  // example linkbox is matched by shape instead, and one that survived would
  // read to the sources stage as a source already added.
  if (doc.hasLinkbox) {
    throw new Error('archetype linkbox not stripped; update entry.js');
  }
