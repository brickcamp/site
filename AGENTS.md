...
the homepage JS on fixtures; `tests/build.test.js` re-runs it on the built
`public/`, so build before trusting a green run.
...
- **Scope catalog.** `_partials/scopes/getCatalog.html` feeds
  ...
  button and the dropdown it links to cannot drift apart.
- **A filter's `from`** says where its values come from, and three unrelated
  readers act on the answer: `_content.gotmpl` gives lookup files to `tags`
  only, `getCatalog` keeps `size` out of the scopes, and `scope.js` spots the
  part scope by `parts`. Deliberately not a lookup page param.
- **Entry fields.** `entry-editor.js`'s schema names the archetype's front-matter
...
- **JS-only CSS classes.** purgecss's allowlist is `hugo_stats.json`: only
  ...
  gitignored and written before `css.PostCSS` reads it — a clean checkout is fine.
...
only; `lookup.js` filters the pipe-joined `values` column, so the file count
follows base and type alone. Don't move it back into the build.
...
- Hugo **publishes no file for a CSV that renders empty**: a scope no entry
  matches has no file, which `lookup.js` reads as no rows.
